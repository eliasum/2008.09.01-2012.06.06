...
////////////////////////////////////////////////////////////////////////////////        
      byte = AUX_DAC;                       // адрес регистра, вывод из которого в последующем цикле      
      SPI_WriteRead(byte);                  // запрос на чтение старшего байта регистра AUX_DAC
      tock.du_sc[1] = SPDR;                 // старший байт данных из TEMP_OUT
      tock.du_sc[1] &= 0x0F;                // данные 12 бит
 
      byte = 0x0A;                          // любые данные
      SPI_WriteRead(byte);                  // запрос на чтение младшего байта регистра AUX_DAC
...
////////////////////////////////////////////////////////////////////////////////           
      ...
      dac.du_c[1] = SPDR;                   // старший байт данных из AUX_DAC
      dac.du_c[1] &= 0x0F;                  // данные 12 бит
 
      byte = 0x0A;                          // любые данные
      SPI_WriteRead(byte);                  // запрос на чтение младшего байта регистра TEMP_OUT
      dac.du_c[0] = SPDR;                   // младший байт данных из AUX_DAC 
      
      uart_puts("Dac/Weight = 0x");         // вывод надписи Dac/Weight = 0x
      uart_puthex_byte(dac.du_c[1]);        // вывод старшего байта данных из AUX_DAC
      uart_puthex_byte(dac.du_c[0]);        // вывод младшего байта данных из AUX_DAC
      uart_puts("/");                       // вывод знака / 
      
      // преобразование тиков в значение веса:
      weight.du_i = (signed int)(0.61*dac.du_i);  
      
      uart_puti(weight.du_i);               // вывод значения веса
      
      uart_putc(0x0d);                      // возврат каретки
      uart_putc(0x0a);                      // перевод на следующую строку 
      delay_ms(200);                        // задержка для усреднения показаний на компьютере
...
////////////////////////////////////////////////////////////////////////////////           
          ...
          uart_putc(weight.du_c[1]);        // вывод угла на БЦК или SMON
          uart_putc(weight.du_c[0]);   
