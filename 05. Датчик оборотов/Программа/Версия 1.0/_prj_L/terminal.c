...
/*----------------------------------------------------------------------------*/
/* ‘ункци€:    ѕреобразование BIN->DEC 's[]'                                  */
/* ѕринимает:  двоичное число                                                 */
/* ¬озвращает: ничего (измен€ет глобальные переменные)                        */
/*----------------------------------------------------------------------------*/
...
{              
   ...
   {
     dnum = -dnum;                      // беретс€ модуль числа
     s[0] = '-';                        // первый выводимый знак "-"
   } 
   else s[0] = '+';                     // первый выводимый знак "+"
                                        
   s[1] = dnum/1000 + '0';              // после знака выводитс€ старший разр€д четырехзначного числа
   s[2] = (dnum%1000)/100 + '0';        // следующий разр€д
   s[3] = ((dnum%1000)%100)/10 + '0';   // следующий разр€д
   s[4] = dnum%10 + '0';                // младший разр€д
