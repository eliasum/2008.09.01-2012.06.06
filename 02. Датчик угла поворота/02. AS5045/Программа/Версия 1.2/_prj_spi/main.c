...
*/      
      tock.du_l = tick.du_l/4096;
      
      uart_puts("Tick/Angle = 0x");         // вывод надписи Tick/Angle = 0x
      uart_puthex_byte(tick.du_c[2]);       // вывод старших 4 бит данных от AS5045
      uart_puts(" ");
      uart_puthex_byte(tick.du_c[1]);       // вывод старших 4 бит данных от AS5045
      uart_puts(" ");
      uart_puthex_byte(tick.du_c[0]);       // вывод следующих 8 бит данных от AS5045
      uart_puts("/");
      
      // преобразование тиков в значение угола наклона:
      angle.du_l = (unsigned int)(0.0879*tock.du_l);
