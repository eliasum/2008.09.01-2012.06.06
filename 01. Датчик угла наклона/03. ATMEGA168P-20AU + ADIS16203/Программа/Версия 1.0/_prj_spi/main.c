...
//////////// Работа в режиме -= УСТАНОВКИ =- /////////////////////////////////
  if (!fl_set)
  {
    ...
    for (;;)
    {
      ...
      byte = 0x0F;                          // запрос на чтение регистра INCL_180_OUT
      SPI_WriteRead(byte);                  // передача запроса
      byte = 0x0E;                          // запрос на чтение регистра INCL_180_OUT
      SPI_WriteRead(byte);                  // передача запроса
...
// Подпрограмма записи/чтеения через SPI
