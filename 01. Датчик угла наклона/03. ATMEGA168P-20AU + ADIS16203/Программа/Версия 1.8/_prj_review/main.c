...
//////////////////////////////////////////////////////////////////////////////// 
      ...
      byte = INCL_180_OUT;                  // адрес регистра, прием данных из которого в следующем цикле
      tick.du_c[1] = SPI_WriteRead(byte);   // старший байт данных INCL_OUT  
      tick.du_c[1] &= 0x3f;                 // данные 14 бит
      
      byte = 0x0C;                          // любые данные
      tick.du_c[0] = SPI_WriteRead(byte);   // младший байт данных INCL_OUT 
...
//////////////////////////////////////////////////////////////////////////////// 
      ...
      tick_180.du_sc[1]=SPI_WriteRead(byte);// старший байт данных INCL_180_OUT  
      tick_180.du_sc[1] &= 0x3F;            // данные 14 бит
      
      byte = 0x0E;                          // любые данные
      tick_180.du_sc[0]=SPI_WriteRead(byte);// младший байт данных INCL_180_OUT 
/*
...
//////////////////////////////////////////////////////////////////////////////// 
      ...
      byte = INCL_OUT;                      // адрес регистра, прием данных из которого в следующем цикле
...
/*    
    Вызов функции get_sens 256 раз перед основным циклом программы избавляет 
    ...
    Число подобрано эмпирически при испытаниях.
*/    
    unsigned int g;
    for(g=0; g<256; g++)                                                                   
...
// Подпрограмма управления фильтрацией
void avg_cnt(void)
{
  unsigned char byte;
////////////////////////////////////////////////////////////////////////////////  
          
                      //Управление фильтрацией (AVG_CNT)
          
////////////////////////////////////////////////////////////////////////////////       
  CS_RES;                                   // активация ведомого
      
  byte = (0x80 + AVG_CNT);                  // адрес регистра, прием данных из которого в следующем цикле
  avg.du_c[1] = SPI_WriteRead(byte);        // старший байт данных AVG_CNT  
      
  byte = 0x08;                              // запись числа 8 для усреднения выходных данных N = 2^AVG_CNT = 2^8 = 256
  avg.du_c[0] = SPI_WriteRead(byte);        // младший байт данных AVG_CNT
      
  CS_SET;                                   // деактивация ведомого
}

// Подпрограмма управления периодом выборок
void smpl_time(void)
{
  unsigned char byte;
////////////////////////////////////////////////////////////////////////////////  
          
                    //Управление периодом выборок (SMPL_TIME)
          
////////////////////////////////////////////////////////////////////////////////       
  CS_RES;                                   // активация ведомого
      
  byte = (0x80 + SMPL_TIME);                // адрес регистра, прием данных из которого в следующем цикле
  smpl.du_c[1] = SPI_WriteRead(byte);       // старший байт данных SMPL_TIME  
      
  byte = 0x01;                              // запись числа 1 для максимальной частоты выдачи данных
  smpl.du_c[0] = SPI_WriteRead(byte);       // младший байт данных SMPL_TIME
