...
/*    
    ...
    unsigned int g;
    for(g=0; g<256; g++)                                                                   
    get_sens();                                // вывод информации, поступающей от ADIS16209
*/    
    for (;;)
    {  
      ...
      if (UCSR0B & (1<<RXB80))                 // если 9-й бит =1, то принята команда
      {
        ...
        {    
          ...
          uart_putc(angle_y.du_sc[1]);         // вывод угла по оси x на БЦК или SMON
...
// Подпрограмма вывода информации, поступающей от ADIS16209
void get_sens(void)
{
...
*/ 
  ...
  // расчет "скользящего среднего":
  //tick_y.du_si = moving_average(tick_y.du_si);
      
  // преобразование тиков в значение угола наклона:
  angle_y.du_si = (signed int)(0.025*tick_y.du_si);     
...
////////////////////////////////////////////////////////////////////////////////       
  ...
  byte = (0x80 + AVG_CNT);                  // адрес регистра, запись в который в следующем цикле
  avg.du_c[1] = SPI_WriteRead(byte);        // старший байт данных из AVG_CNT  
      
  byte = 0x08;                              // запись числа 8 для усреднения выходных данных N = 2^AVG_CNT = 2^8 = 256
  avg.du_c[0] = SPI_WriteRead(byte);        // младший байт данных из AVG_CNT
...
////////////////////////////////////////////////////////////////////////////////       
  ...
  byte = (0x80 + SMPL_TIME);                // адрес регистра, запись в который в следующем цикле
  smpl.du_c[1] = SPI_WriteRead(byte);       // старший байт данных из SMPL_TIME  
      
  byte = 0x01;                              // запись числа 1 для максимальной частоты выдачи данных
  smpl.du_c[0] = SPI_WriteRead(byte);       // младший байт данных из SMPL_TIME
